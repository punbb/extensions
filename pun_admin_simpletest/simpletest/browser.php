...
class SimpleBrowser {
    ...
    function get($url, $parameters = false) {
		...
		$parts_arr_url = array();
		$parts_arr_url = parse_url($url);
		
		if ($parts_arr_url['path'] == '')
		{
			$url .= '/?simpletest='.$number;
		}
		else if ($parts_arr_url['query'] == '')
			$url .= '?simpletest='.$number;
		else
			$new_attr_url = $parts_arr_url['query'].'&simpletest='.$number;
		
		$url = str_replace($parts_arr_url['query'], $new_attr_url, $url);
		
		echo '<pre>page browser.php function get URL ';
		print_r($url);
		echo '</pre>';
    ...
    function post($url, $parameters = false) {
		...
		echo '<pre>page browser.php function post URL ';
		print_r($url);
		echo '</pre>';
