...
class SimpleForm {
    ...
    function _createAction($action, &$page) {
		...
		if ($page)
		{
			$n_attr = count($page->_links);
			
			for ($iter = 0; $iter < $n_attr; $iter++)
			{
				if ($page->_links[$iter]->_attributes['href'] != '#brd-main')
				{
					$line_tmp_st = $page->_links[$iter]->_attributes['href'];
					
					if (strstr($url, 'simpletest') == false)
					{
						if (strpos($line_tmp_st, '.ru/', (strlen($line_tmp_st) - 5)))
							$line_tmp_st .= '?simpletest='.$number;
						else if (strpos($line_tmp_st, '?'))
							$line_tmp_st .= '&simpletest='.$number;
						else if (strpos($line_tmp_st, '.php', (strlen($line_tmp_st) - 5)))
							$line_tmp_st .= '?simpletest='.$number;
						else
							$line_tmp_st .= '/?simpletest='.$number;
					}
					
					$page->_links[$iter]->_attributes['href'] = $line_tmp_st;
		...
		if (!preg_match('/simpletest/', $action))
		{
			if (strpos($action, '.ru/', (strlen($action) - 5)))
				$action .= '?simpletest='.$number;
			else if (strpos($action, '?'))
				$action .= '&simpletest='.$number;
			else if (stripos($action, '.php', (strlen($action) - 5)))
				$action .= '?simpletest='.$number;
			else
				$action .= '/?simpletest='.$number;
		}
		
		echo '<pre>page form.php This is action ';
		print_r($action);
		echo '</pre>';
