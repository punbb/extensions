...
class SimpleTestCase {
     ...
     */
    function setUp() {
		$path_file = $_SERVER['DOCUMENT_ROOT'].'/extensions/pun_admin_simpletest/cache_simpletest.php';
		
		if (file_exists($path_file))
			if (!unlink($path_file))
				die('BBBBBBBBLLLLLLLLLLLLLLLLLYYYYYYYYYYYYYYYYAAAAAAAAAAAAAAAAAAAAAAAA');
		
		$number = mt_rand(1000000, 40000000);
		
		$handle_file = fopen($path_file, 'w');
		fwrite($handle_file, $number);
		fclose($handle_file);
    }
