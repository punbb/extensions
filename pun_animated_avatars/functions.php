...
{
	global $forum_url, $forum_config, $errors, $lang_pun_animated_avatars;
...
{
	global $forum_url, $errors, $forum_config, $lang_pun_animated_avatars, $forum_user;
...
function visit_pho_to_page($user_id, $request_id)
{
	...
	if (!empty($get_result_response['content']))
	{
		...
		$errors[] = sprintf($lang_pun_animated_avatars['Pho.to error get response'], forum_link($forum_url['edit_avatar'], array($user_id)).'&amp;request_id='.$request_id);
	...
	if (empty($errors) && !empty($get_result_response['image_process_response']['status']))
	{
		switch ($get_result_response['image_process_response']['status'])
		{
			case 'InProgress':
				...
				$errors[] = sprintf($lang_pun_animated_avatars['Pho.to error task in progress'], forum_link($forum_url['edit_avatar_request'], array($user_id, $request_id, generate_form_token('request_id'.$forum_user['id']))));
				break;
			case 'Error':
