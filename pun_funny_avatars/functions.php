...
function pun_funny_avatars_gen_link($url, $args)
{
...
function pun_funny_avatars_generate_templates_cache()
{
	global $forum_url;
	...
	$fh = @fopen(FORUM_CACHE_DIR.'cache_pun_funny_avatars.php', 'wb');
	if (!$fh)
	...
	fwrite($fh, '<?php'."\n\n".'if (!defined(\'FUNNY_TEMPLATES_LOADED\')) define(\'FUNNY_TEMPLATES_LOADED\', 1);'."\n\n".'$funny_templates = '.var_export($pho_to_templates, true).';'."\n\n".'?>');

	fclose($fh);
}

function pun_funny_avatars_template($template, $user_id, $type = 'jpg', $auto_crop = 'FALSE')
{
	global $forum_url, $errors, $forum_config, $lang_pun_funny_avatars, $forum_user;

	$queue_response = get_remote_file(pun_funny_avatars_gen_link($forum_url['pho.to_FET_queue'], array(PUN_FUNNY_AVATARS_FREE_KEY, forum_link($forum_config['o_pun_funny_avatars_file_dir'].'/'.$user_id.'.'.$type), PUN_FUNNY_AVATARS_IMAGE_LIMIT, $template, $auto_crop, min($forum_config['o_avatars_width'], $forum_config['o_avatars_height']))), 10);
	if (!empty($queue_response['content']))
	{
		...
		if (strtolower($get_response['image_process_response']['status']) == 'ok' && !empty($get_response['image_process_response']['request_id']))
			pun_funny_avatars_visit_pho_to_page($user_id, $get_response['image_process_response']['request_id']);
		else
...
function pun_funny_avatars_visit_pho_to_page($user_id, $request_id)
{
	global $forum_url, $errors, $lang_pun_funny_avatars, $forum_user;

	$get_result_response = get_remote_file(pun_funny_avatars_gen_link($forum_url['pho.to_get-result'], array($request_id)), 10);
	if (!empty($get_result_response['content']))
	{
		...
		$errors[] = sprintf($lang_pun_funny_avatars['Pho.to error get response'], forum_link($forum_url['funny_avatar'], array($user_id)).'&amp;request_id='.$request_id);
	...
	if (empty($errors) && !empty($get_result_response['image_process_response']['status']))
	{
		switch ($get_result_response['image_process_response']['status'])
		{
			case 'InProgress':
				usleep(170000);
				header('Location: '.str_replace('&amp;', '&', forum_link($forum_url['funny_avatar_request'], array($user_id, $request_id, generate_form_token('request_id'.$forum_user['id'])))));
				$errors[] = sprintf($lang_pun_funny_avatars['Pho.to error task in progress'], forum_link($forum_url['funny_avatar_request'], array($user_id, $request_id, generate_form_token('request_id'.$forum_user['id']))));
				break;
			case 'Error':
		...
		if (empty($errors))
			header('Location: '.$get_result_response['image_process_response']['page_to_visit'].'&redirect_url='.urlencode(str_replace('&amp;', '&', forum_link($forum_url['funny_avatar'], array($user_id)))));
	}
	else
		$errors[] = $lang_pun_funny_avatars['Pho.to error server'];
}

function pun_funny_avatars_get_avatar_type($user_id)
{
	global $forum_config;
...
function pun_funny_avatars_get_file_type($user_id)
{
	global $forum_config;
	...
	foreach ($filetypes as $cur_type)
	{
		if (file_exists(FORUM_ROOT.$forum_config['o_pun_funny_avatars_file_dir'].'/'.$user_id.'.'.$cur_type))
			return $cur_type;
	}
...
function pun_funny_avatars_upload_photo($id)
{
	...
	if (is_uploaded_file($uploaded_file['tmp_name']) && empty($errors))
	{
		...
		else
		{
			...
			if ($uploaded_file['size'] > $forum_config['o_pun_funny_avatars_max_size'])
				$errors[] = sprintf($lang_profile['Too large'], forum_number_format($forum_config['o_pun_funny_avatars_max_size']));
		}
		...
		if (empty($errors))
		{
			...
			if (!@move_uploaded_file($uploaded_file['tmp_name'], $forum_config['o_pun_funny_avatars_file_dir'].'/'.$id.'.tmp'))
			...
			if (empty($errors))
			{
				...
				list($width, $height, $type,) = getimagesize($forum_config['o_pun_funny_avatars_file_dir'].'/'.$id.'.tmp');
				if (empty($width) || empty($height) || $width > $forum_config['o_pun_funny_avatars_max_width'] || $height > $forum_config['o_pun_funny_avatars_max_height'])
				{
					@unlink($forum_config['o_pun_funny_avatars_file_dir'].'/'.$id.'.tmp');
					$errors[] = sprintf($lang_profile['Too wide or high'], $forum_config['o_pun_funny_avatars_max_width'], $forum_config['o_pun_funny_avatars_max_height']);
				}
...
function pun_funny_avatars_remove_photo($user_id)
{
	...
	$type = pun_funny_avatars_get_file_type($user_id);
	if (!$type)
		message($lang_common['Bad request']);
	unlink(FORUM_ROOT.$forum_config['o_pun_funny_avatars_file_dir'].'/'.$user_id.'.'.$type);

	$query = array(
		'DELETE'	=>	'pun_funny_avatars_files',
