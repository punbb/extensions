	...
	if (count($forum_page['list']))
	{
		...
		foreach($forum_page['list'] as $message)
		{
			...
			$message_info = array (
				...
				'<span><a href="'.$message_link.'">'.trim($message['subject'] ? forum_htmlencode($message['subject']) : $lang_pun_pm['Empty']).'</a>'.($forum_user['pun_pm_long_subject'] == '1' ? ' <a class="mess" href="'.$message_link.'">'.forum_htmlencode(preg_replace('#((?:\S*\s*){20})(?:.*)$#su', '$1', $message['body'])).'</a>' : '').'</span>',
