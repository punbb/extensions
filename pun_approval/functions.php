...
function show_unapproved_posts()
{
	...
	if ($aptid || $appid)
	{
		...
		$query = array(
			'SELECT'	=> 'DISTINCT unp.id, unp.poster_id AS unp_poster_id, unp.topic_id AS unp_topic_id, unp.posted AS unp_posted, unp.message AS unp_message, t.subject AS top_subject, t.poster AS top_poster, t.posted AS top_posted, u.username, u.registered, t.subject, u.email',
			'FROM'		=> 'post_approval_posts AS unp',
			'JOINS'		=> array(
				array(
					'LEFT JOIN'		=> 'topics AS t',
					'ON'			=> '(unp.topic_id = t.id AND t.id='.$aptid.')'
				),
				array(
