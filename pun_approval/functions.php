...
{
	global $forum_db, $forum_user, $forum_url, $lang_common, $lang_app_post, $forum_config, $lang_forum, $lang_topic, $base_url, $forum_page, $cur_forum, $ext_info, $smilies, $attach_url, $lang_attach;
	...
	<div class="main-content main-forum">
		...
		<?php
		$app_row = array();
		while ($cur_row = $forum_db->fetch_assoc($result_app_topic)) {
			$app_row[] = $cur_row;
		}

		if (!empty($app_row)) {
			$forum_page['item_count'] = 0;
			foreach ($app_row as $cur_topic) {
				++$forum_page['item_count'];
			...
			<div class="main-subhead">
				<h3 class="hn"><span><a
					href="<?php echo forum_link($post_app_url['Posts section']) ?>"><?php echo $lang_app_post['See posts']?></a></span></h2>
			</div>
			<?php
		}
		else {
	...
	if ($aptid || $appid || $all_post) {
		...
		$query = array(
			...
			'WHERE'		=> '(fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$topics.' AND t.moved_to IS NULL'
		);
		...
		<?php
			...
			if (!$all_post && $aptid) {
				if (in_array('pun_poll', $installed_ext)) //check if pun_pooll is installed
				{
					...
					$app_row = array();
					while ($cur_row = $forum_db->fetch_assoc($poll_question_result)) {
						$app_row[] = $cur_row;
					}
			...
			foreach ($app_row as $cur_post) {
				...
				if (in_array('pun_attachment', $installed_ext)) {
					if (!$forum_config['attach_disable_attach']) {
						...
						while ($cur_row = $forum_db->fetch_assoc($attach_result)) {
							$app_row[] = $cur_row;
						}

						if (!empty($app_row)) {
							$attach_list = array();
							foreach ($app_row as $cur_attach) {
...
function delete_unapproved_user()
{
	...
	if ($forum_config['o_send_rej_email'] == '0') {
		...
		if (file_exists($ext_info['path'].'/lang/'.$forum_user['language'].'/mail_templates/reg_declined.tpl'))
			$mail_tpl = forum_trim(file_get_contents($ext_info['path'].'/lang/'.$forum_user['language'].'/mail_templates/reg_declined.tpl'));
		else
			$mail_tpl = forum_trim(file_get_contents($ext_info['path'].'/lang/English/mail_templates/reg_declined.tpl'));
...
function delete_unapproved_post()
{
	...
	if (!$confirmation_required) {
		...
		if (!empty($app_row)) //if we are deleting a first post of some topic, we should delete all posts with this topic_id.
		{
			$query_app = array(
			...
			if (in_array('pun_attachment', $installed_ext)) {
				...
				$app_row = array();
				while ($cur_row = $forum_db->fetch_assoc($result_att_filepath)) {
					$app_row[] = $cur_row;
				}
				if (!empty($app_row)) {
					foreach ($app_row as $cur_row) {
		...
		else //if we are deleteing a single post
		{
			...
			$result_app_post = $forum_db->query_build($query_app_post) or error(__FILE__, __LINE__);
			$app_row = array();
			while ($cur_row = $forum_db->fetch_assoc($result_app_post)) {
				$app_row[] = $cur_row;
			}
			if (!empty($app_row)) {
				foreach ($app_row as $cur_row) {
...
function approve_user()
{
	...
	if ($forum_config['o_regs_verify'] == '1') {
		...
		if (file_exists($ext_info['path'].'/lang/'.$forum_user['language'].'/mail_templates/reg_approved.tpl'))
			$mail_tpl = forum_trim(file_get_contents($ext_info['path'].'/lang/'.$forum_user['language'].'/mail_templates/reg_approved.tpl'));
		else
			$mail_tpl = forum_trim(file_get_contents($ext_info['path'].'/lang/English/mail_templates/reg_approved.tpl'));
...
function approve_post()
{
	...
	else //if this post is NOT the first post of the topic
	{
		...
		$num_replies = app_count_tid($topics);
		// Update topic
		$query = array(
			'UPDATE'	=> 'topics',
			'SET'		=> 'num_replies='.$num_replies.', last_post='.$post_info['posted'].', last_post_id='.$new_pid.', last_poster=\''.$forum_db->escape($post_info['poster']).'\'',
		...
		$num_replies = app_count_tid_approval($topics);
